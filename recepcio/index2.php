...
html_start("Maverick Reception - Activities for today ($today)", $extraHeader, true, 'initSendBcr();', true);
...
<form class="form-inline" action="index.php">
	<div class="form-group">
		<label for="currentDate">Set current date to: </label>
		<input type="date" class="form-control" id="currentDate" value="$dayToShow">
	</div>
	<button type="submit" class="btn btn-default">Set active day</button>
</form><br><br>
	...
	if(isset($guestData[$id])) {
		foreach($guestData[$id] as $oneGD) {
			if($currentGuestOrder == 'room' or $currentGuestOrder == 'guest_name' or $currentGuestOrder == 'guest_deposit') {
				...
				$dataArr[] = array($id, $name, $fnight, $lnight, $gdRoom, $oneGD['name'], $oneGD['deposit'], $class);
			} else {
		...
		if($currentGuestOrder != 'room' and $currentGuestOrder != 'guest_name' and $currentGuestOrder != 'guest_deposit') {
			$dataArr[] = array($id, $name, $fnight, $lnight, $gdRoom, $gdName, $gdDeposit, $class);
		}
		...
		if($currentGuestOrder != 'room') {
			$dataArr[] = array($id, $name, $fnight, $lnight, $roomCol, '', '', $class);
		}
	}
...
foreach($dataArr as $row) {
	...
	$gdDeposit = $row[6];
	$class = $row[7];
	echo "	<tr class=\"$class\"><td><a href=\"edit_booking.php?description_id=$id\">$name</a></td><td>$fnight</td><td>$lnight</td><td>$room</td><td>$gdName</td><td>$gdDeposit</td></tr>\n";
