<?php

define('DB_HOST', 'localhost');
define('DB_USER', 'maverick');
define('DB_PASSWORD', 'maverick');
define('DB_NAME', 'maverickhos_3637');

define('PHP_MAILER_LANGUAGE_DIR', "c:/Projects/maverick/recepcio/includes/phpmailer/language");
define('PHP_MAILER_SENDTYPE_SENDMAIL', 'SENDMAIL');
define('PHP_MAILER_SENDTYPE_MAIL', 'MAIL');
define('PHP_MAILER_SENDTYPE_SMTP', 'SMTP');
define('PHP_MAILER_SENDTYPE', PHP_MAILER_SENDTYPE_SMTP);

define('PHP_MAILER_SENDTYPE_SMTP_HOST', 'mail.t-online.hu');
define('PHP_MAILER_SENDTYPE_SMTP_PORT', '25');
define('PHP_MAILER_SENDTYPE_SMTP_AUTHORIZATION_REQ', true);
define('PHP_MAILER_SENDTYPE_SMTP_SECURE', '');
define('PHP_MAILER_SENDTYPE_SMTP_USER', 'zoltanfulop74@t-online.hu');
define('PHP_MAILER_SENDTYPE_SMTP_PASSWORD', 'gsm4nmH7');

define('EMAIL_IMG_DIR', "c:/Projects/maverick/www/img/email/");

define('IMG_ROOT_URL', 'http://localhost-maverick/img/');
define('IMG_ROOT_DIR', 'c:/Projects/maverick/www/img/');

define('ROOMS_IMG_DIR', IMG_ROOT_DIR . "rooms/");
define('ROOMS_IMG_URL', IMG_ROOT_URL . "rooms/");
define('SERVICES_IMG_DIR', IMG_ROOT_DIR . "services/");
define('SERVICES_IMG_URL', IMG_ROOT_URL . "services/");
define('AWARDS_IMG_DIR', IMG_ROOT_DIR . "awards/");
define('AWARDS_IMG_URL', IMG_ROOT_URL . "awards/");

define('ROOT_URL', '/');
define('LOCATION', 'lodge');
define('CONTACT_EMAIL', 'reservation@mavericklodges.com');

define('EXCHANGE_TABLE_FILE', 'c:/Projects/maverick/recepcio/includes/exchange_table.php');

define('FPDF_FONTPATH','c:/Projects/maverick/recepcio/includes/font');

define('CONFIRM_BOOKING_URL', 'http://localhost-maverick/LANG/confirm_booking.php?CONFIRM_CODE');
define('LANG_DIR', 'c:/Projects/maverick/www/includes/language/');

?>
