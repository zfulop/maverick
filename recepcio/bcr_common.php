...
class BCR {
	...
	function __construct($bookingDescr, $location, &$dict, $link) {
		...
		$this->descrId = $bookingDescr['id'];
		$this->currency = $bookingDescr['currency'];
		if(is_null($this->currency) or strlen(trim($this->currency)) < 3) {
			$this->currency = 'EUR';
		}
		$this->lang = $bookingDescr['language'];
		...
		$this->dict = $dict;
		$this->total = 0;
		$this->$roomArray = loadRooms($link);
		$this->$serviceArray = loadServices($link);
		$this->$paymentArray = loadPayments($link);
	}
	...
	private function loadRooms($link) {
		...
		$sql = "
			SELECT b.*, l.value AS room_name, rt.type AS room_type 
			FROM bookings b 
	...
	private function loadServices($link) {
		...
		$sql = "
			SELECT sc.*, s.price, s.currency AS svcCurr, l.value AS title 
	...
	private function loadPayments($link) {
		...
		while($row = mysql_fetch_assoc($result)) {
			...
			$this->paymentArray[] = array('item_name' => $title,
									'item_price' => $amount . $this->currency);
		}	

	}
	
	function sendBookingMessageToReception($link) {
	...
	function sendBcrMessage($subject, $bcrMessage, $link) {
		...
		$locationName = $this->dict[$this->lang]['LOCATION_NAME_' . strtoupper($this->location)];
		...
		$mailer = new MaverickMailer(CONTACT_EMAIL, $locationName, $this->bookingDescr['email'], $this->bookingDescr['name']);
