...
if(!$result) {
	...
	$dayCloseHuf = $lastDayClose['casseHUF2'];
	$dayCloseEur = $lastDayClose['casseEUR2'];
}

$sql = "SELECT payments.*, booking_descriptions.name FROM payments LEFT OUTER JOIN booking_descriptions ON payments.booking_description_id=booking_descriptions.id WHERE payments.time_of_payment>'" . START_DATE . "' AND payments.pay_mode<>'CASH'";
if(!is_null($viewDay)) {
	$sql .= " AND SUBSTR(payments.time_of_payment,1,10)='$viewDay'";
} else {
	$sql .= " AND payments.time_of_payment>'$lastDayCloseTime'";
}
...
if(!$result) {
	...
	while($row = mysql_fetch_assoc($result)) {
		...
		$payments[] = $row;
		if(($row['pay_mode'] == 'CASH2' or $row['pay_mode'] == 'CASH3') and $row['storno'] != 1) {
			if($row['currency'] == 'EUR')
				$eurCasse += $row['amount'];
			else
				$hufCasse += $row['amount'];
...
if(!is_null($viewDay)) {
	$sql = "SELECT * FROM cash_out WHERE time_of_payment>'" . START_DATE . "' AND SUBSTR(time_of_payment,1,10)='$viewDay' AND pay_mode<>'CASH' ORDER BY time_of_payment";
} else {
	$sql = "SELECT * FROM cash_out WHERE time_of_payment>'" . START_DATE . "' AND time_of_payment>'$lastDayCloseTime' AND pay_mode<>'CASH'  ORDER BY time_of_payment";
}
$cashOuts = array();
$result = mysql_query($sql, $link);
if(!$result) {
...
$cashoutEur = 0;
$cashoutHuf = 0;
while(count($payments) > $pymtIdx or count($cashOuts) > $coIdx) {
	$p = null;
	$c = null;
	$g = null;
	$arr = array();
	...
	if($takePymt) {
		...
		$cashTr = (($payment['pay_mode'] == 'CASH2') or ($payment['pay_mode'] == 'CASH3'));
		...
		$id = $co['id'];
		$storno = $co['storno'] == 1;
		$cashTr = (($co['pay_mode'] == 'CASH2') or ($co['pay_mode'] == 'CASH3'));
